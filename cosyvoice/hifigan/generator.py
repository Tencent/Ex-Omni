...
class HiFTGenerator(nn.Module):
    ...
    def _stft(self, x):
        ...
        # 返回时移回原始设备
        return spec[..., 0].to(device), spec[..., 1].to(device)
    ...
    def _istft(self, magnitude, phase):
        ...
        magnitude_cpu = torch.clip(magnitude_cpu, max=1e2)
        real = magnitude_cpu * torch.cos(phase_cpu)
        imag = magnitude_cpu * torch.sin(phase_cpu)
