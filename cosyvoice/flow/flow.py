...
class MaskedDiffWithXvec(torch.nn.Module):
    ...
    def inference1(self,
        ...
        # get conditions - 修改以支持多batch
        max_feat_len = feat_len.max().item()
        conds = torch.zeros([batch_size, max_feat_len, self.output_size], device=token.device)
        
        # 处理每个batch的prompt_feat
        if prompt_feat.shape[1] != 0:
            for i, j in enumerate(prompt_feat_len):
                if j > 0:  # 添加长度检查
                    conds[i, :j] = prompt_feat[i, :j]  # 确保不越界
        conds = conds.transpose(1, 2)
        ...
        # 处理多batch的prompt_feat移除
        if prompt_feat.shape[1] != 0:
            # 为每个batch单独处理，移除prompt部分
            batch_results = []
            for i in range(batch_size):
                prompt_len = prompt_feat_len[i].item() if prompt_feat_len[i] > 0 else 0
                if prompt_len > 0:
                    batch_results.append(feat[i:i+1, :, prompt_len:])
                else:
                    batch_results.append(feat[i:i+1])
            feat = torch.cat(batch_results, dim=0)
