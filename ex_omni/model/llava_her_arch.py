...
from ex_omni.constants import IGNORE_INDEX , SPEECH_TOKEN_INDEX
...
class LlavaHerMetaForCausalLM(ABC):
    ...
    def encode_speech(self, speech, speech_lengths):
        ...
        target_device = self.device  # 或 next(self.parameters()).device
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            if num_speech == 0:
                ...
                empty_speech_features = cur_speech_features[0:0].to(device=cur_input_embeds_1.device,dtype=cur_input_embeds_1.dtype)
                cur_input_embeds = torch.cat([cur_input_embeds_1, empty_speech_features], dim=0)
            ...
            for i in range(len(speech_token_indices) - 1):
                cur_input_ids_nospe.append(cur_input_ids[speech_token_indices[i]+1:speech_token_indices[i+1]])
                cur_labels_nospe.append(cur_labels[speech_token_indices[i]+1:speech_token_indices[i+1]])
